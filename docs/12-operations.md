...
### اختبار الاستعادة من نسخة مشفرة (شهريًا — معيار قبول v2)

```bash
cd /opt/hrms
bash scripts/backup.sh                                  # إنشاء نسخة (مشفرة AES-256)
.venv/bin/python manage.py restore_db backups/hrms_*.enc --dry-run   # فك تشفير + تحقق دون تعديل
```

- الاختبار يُسجَّل في جدول `audit_restorejob` (تقرير من لوحة سجل التدقيق).
- أي اختبار **فعلي** (غير `--dry-run`) يجب أن يكون في نافذة صيانة، مع نسخة احتياطية جديدة أولًا.

---

## 3.1 التحقق من الأداء (اختبار 5000 موظف)

إجراء موثق لاختبار الأداء قبل توسيع القاعدة الفعلية:

```bash
cd /opt/hrms
DJANGO_SETTINGS_MODULE=config.settings.prod .venv/bin/python manage.py seed_demo --employees 5000
```

1. شغّل على **PostgreSQL** (وليس SQLite) في بيئة معزولة/نظيفة.
2. قِس زمن الاستجابة باستخدام `curl -w "%{time_total}\n"` للشاشات الحرجة:
   - لوحة التحكم، قائمة الموظفين، تقرير حضور شهري (REP-10/12)، تقرير رواتب (REP-30).
3. الحد المقبول (NFR): استجابة < 1.5s للمستخدم العادي، < 4s للتقارير الثقيلة على شبكة LAN مع < 50 مستخدم متزامن.
4. عند أي تجاوز: تفعيل الفهارس/Partitioning (انظر `docs/03-database-design.md`، `docs/10-roadmap.md` §9) ثم إعادة الاختبار.
5. سجّل النتيجة في ملاحظات الإصدار (تاريخ، أجهزة، أزمنة).
